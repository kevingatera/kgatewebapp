    ...
    router.post('/users', function(req, res){
        ...
        var missingCriteria = function(){
            for(i in userData){
                ...
                if(userData[i] == ('' || null)) { flagForMissing = true}
...
/* USER LOGIN ROUTE */
    ...
    router.post('/authenticate', function(req, res){
        ...
        /* the following will fid the account by username */
        User.findOne({
            ...
            if(err) {
                ...
                if(!user) {
                    res.json({
                        success: false,
                        message: 'Could not authenticate the username'
                    })
                } else if (user) {
                    if (!req.body.password) {
                        ...
                        if (!validPassword) {
                            res.json({
                                success: false,
                                message: 'Could not authenticate the password'
                            })
    ...
    router.use(function(req, res, next){
        ...
        if (token){
            ...
            res.json({
                success: false,
                message: 'Failure: NO token provided'
            })
        }
    })
